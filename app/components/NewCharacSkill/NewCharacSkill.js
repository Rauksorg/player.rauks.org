...
// Chnage skilllist back to array list with only skill id create a flat detailed skill list
             ...
             <List>
              {Object.keys(id.skills).map((skillId, index) => {
                ...
                return (
                  <ListItem
                    key={index}
                    leftCheckbox={<Checkbox value={true} onCheck={(event, isInputChecked) => {if(isInputChecked){addSkill(skillId)}else{removeSkill(skillId)}
                    console.warn(skillsList.has(skillId))}}/>}
